...
def _commit_or_translate(db: Session) -> None:
    """Commit and translate IntegrityError into a clean 422.
...
def _release_order_reservations(
    ...
    Skips items whose reservation has already been released — this is
    a defensive no-op so a cancel cannot accidentally push
    ``quantity_reserved`` negative on a stock that was somehow
    consumed first. The state machine guarantees a delivered order
    cannot reach this path, so in practice the skip never fires.
    """
    for line in order.items:
...
def create_order(
    ...
    *,
    ...
    try:
        ...
        message = str(exc.orig).lower() if exc.orig is not None else ""
        if "uq_orders_store_idempotency_key" in message:
            # Race: another request committed first. Re-fetch.
    ...
    db.refresh(order)
    # Make sure the .items collection reflects the freshly-flushed rows.
...
def transition_order_status(
    ...
    if payload.new_status == OrderStatus.delivered:
        ...
        # Pure status-only transition. Set the matching lifecycle
        # timestamp for transitions that have one.
