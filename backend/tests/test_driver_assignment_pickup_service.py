"""Dr.1.1.L — driver pickup service tests (L-mínima / operational-only).

Exercises `pickup_driver_assignment`: the operational state
`arrived_at_store -> picked_up` transition, idempotency (no timestamp rewrite),
the 422 invalid-source-status guard for every non-started assignment status,
the 422 "not yet arrived at store" guard (missing / behind state — pickup NEVER
materializes a row), the 409 "already past picked_up" / 422 terminal guards, the
anti-enumeration 404 boundary, and the no-side-effects guarantee (no
Order.status / inventory / assignment-status / assignment-timestamp bleed).
...
def make_store(db_session: Session) -> Callable[..., Store]:
    def _create(name: str = "DPU-Store") -> Store:
        store = Store(name=name, code=f"dpu-{uuid.uuid4().hex[:8]}")
...
def test_pickup_from_arrived(db_session: Session, setup) -> None:
    ...
    result = pickup_driver_assignment(db_session, user, assignment.id)
    ...
    # Pickup NEVER mutates the assignment status.
...
def test_pickup_idempotent_when_already_picked_up(
...
def test_pickup_invalid_assignment_status_422(
    ...
    with pytest.raises(HTTPException) as exc:
        pickup_driver_assignment(db_session, user, assignment.id)
    assert exc.value.status_code == 422
    assert "Assignment cannot pickup from status" in exc.value.detail
    ...
    assert state.state == "arrived_at_store"


# --------------------------------------------------------------------- #
# Operational state missing -> 422 (not yet arrived; never materializes)
# --------------------------------------------------------------------- #


def test_pickup_without_operational_state_422(
    db_session: Session, setup
) -> None:
    _store, user, _profile, assignment = setup("started")
    ...
    with pytest.raises(HTTPException) as exc:
        pickup_driver_assignment(db_session, user, assignment.id)
    assert exc.value.status_code == 422
    assert exc.value.detail == "Delivery not yet arrived at store"

    # Pickup NEVER materializes the operational-state row.
...
@pytest.mark.parametrize(
    "behind_state",
    ["not_started", "en_route_to_store", "pickup_started"],
)
...
def test_pickup_ahead_state_409(
    ...
    assert exc.value.detail == "Delivery already past picked_up"
...
def test_pickup_nonexistent_404(db_session: Session, setup) -> None:
    _store, user, _profile, _assignment = setup("started")
    with pytest.raises(HTTPException) as exc:
        pickup_driver_assignment(db_session, user, uuid.uuid4())
    assert exc.value.status_code == 404
    assert exc.value.detail == "Driver assignment not found"


def test_pickup_other_driver_404(db_session: Session, make_store) -> None:
    store = make_store()
    ...
    with pytest.raises(HTTPException) as exc:
        pickup_driver_assignment(db_session, other, assignment.id)
    assert exc.value.status_code == 404
    assert exc.value.detail == "Driver assignment not found"
...
def test_pickup_other_store_404(db_session: Session, make_store) -> None:
...
def test_pickup_driver_without_profile_404(
...
def test_pickup_no_order_inventory_or_assignment_mutation(
    db_session: Session, setup
) -> None:
    ...
    # Pickup touches ONLY the operational state — never the assignment.
