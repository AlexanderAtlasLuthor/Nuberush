"""Dr.1.1.M — driver depart-to-customer API tests (operational-only).

Confirms POST /driver/assignments/{id}/depart-to-customer: the 200 happy path
and response shape (DriverDeliveryOperationalStateRead, no PII / no Order.status
/ no inventory), idempotency with preserved timestamps, the anti-enumeration
404s, the role/store/auth gates, and the 409/422 guards.
"""
...
def _depart_url(assignment_id) -> str:
    return f"/driver/assignments/{assignment_id}/depart-to-customer"
...
def make_store(db_session: Session) -> Callable[..., Store]:
    def _create(name: str = "DDC-Api") -> Store:
        store = Store(name=name, code=f"ddca-{uuid.uuid4().hex[:8]}")
...
def test_depart_200_shape(
    ...
    resp = client.post(_depart_url(assignment.id), headers=_auth(user))
    assert resp.status_code == 200, resp.text
    body = resp.json()
    assert set(body.keys()) == _STATE_KEYS
    assert body["state"] == "en_route_to_customer"
...
def test_depart_response_no_pii_or_order_fields(
    client: TestClient, db_session: Session, make_store
) -> None:
    store = make_store()
    user = _driver(db_session, store)
    ...
    body = client.post(
        _depart_url(assignment.id), headers=_auth(user)
    ).json()
    for forbidden in (
        "user",
        "email",
...
def test_depart_idempotent_preserves_timestamps(
    client: TestClient, db_session: Session, make_store
) -> None:
    store = make_store()
    user = _driver(db_session, store)
    ...
    first = client.post(_depart_url(assignment.id), headers=_auth(user))
    second = client.post(_depart_url(assignment.id), headers=_auth(user))
    assert first.status_code == 200
    assert second.status_code == 200
    assert first.json()["state"] == "en_route_to_customer"
    assert (
...
def test_depart_invalid_status_422(
...
def test_depart_without_state_422(
    client: TestClient, db_session: Session, make_store
) -> None:
    store = make_store()
    user = _driver(db_session, store)
    ...
    resp = client.post(_depart_url(assignment.id), headers=_auth(user))
    assert resp.status_code == 422, resp.text
    assert resp.json()["detail"] == "Delivery not yet picked up"


@pytest.mark.parametrize(
    "behind_state",
    ["not_started", "en_route_to_store", "arrived_at_store", "pickup_started"],
)
...
def test_depart_ahead_state_409(
    client: TestClient, db_session: Session, make_store, ahead_state: str
) -> None:
    store = make_store()
    user = _driver(db_session, store)
    _profile, _order, assignment = _owned_assignment(db_session, store, user)
    ...
    assert resp.json()["detail"] == "Delivery already past en_route_to_customer"


@pytest.mark.parametrize(
    "terminal_state",
    ["delivery_completed", "delivery_failed", "returned_to_store", "canceled"],
)
...
def test_depart_404_nonexistent(
    client: TestClient, db_session: Session, make_store
) -> None:
    store = make_store()
    user = _driver(db_session, store)
    make_driver_profile(db_session, user=user, store=store)
    resp = client.post(_depart_url(uuid.uuid4()), headers=_auth(user))
    assert resp.status_code == 404, resp.text
    assert resp.json()["detail"] == "Driver assignment not found"


def test_depart_404_other_driver(
    client: TestClient, db_session: Session, make_store
) -> None:
    store = make_store()
    ...
    resp = client.post(_depart_url(assignment.id), headers=_auth(other))
    assert resp.status_code == 404, resp.text


def test_depart_404_other_store(
    client: TestClient, db_session: Session, make_store
) -> None:
...
def test_depart_non_driver_403(
...
def test_depart_storeless_driver_403(
...
def test_depart_anonymous_401(client: TestClient) -> None:
    assert client.post(_depart_url(uuid.uuid4())).status_code == 401
