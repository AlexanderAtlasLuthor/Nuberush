...
describe("resolveApiBaseUrl", () => {
  ...
  it("throws in production when env is missing or blank", () => {
    const message = "VITE_API_BASE_URL is required for production builds.";
    expect(() => resolveApiBaseUrl(undefined, true)).toThrow(message);
    expect(() => resolveApiBaseUrl(null, true)).toThrow(message);
    expect(() => resolveApiBaseUrl("", true)).toThrow(message);
    expect(() => resolveApiBaseUrl("   ", true)).toThrow(message);
  });
