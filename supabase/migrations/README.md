...
| `20260530120000_store_applications_rls.sql` | F2.24.C1 | Extends the F2.22.3.D deny-all baseline to the two new merchant store-application tables created by Alembic revision `e3b1d4c7a9f2` — `public.store_applications` and `public.store_application_audit_logs`. `ENABLE` + `FORCE` ROW LEVEL SECURITY only; no positive policies for `authenticated` or `anon`. The F2.24 public intake endpoint is unauthenticated but writes through FastAPI's `nuberush_app` BYPASSRLS role, never `supabase-js`, so denying direct table access keeps merchant applications API-only even if an anon key leaks. |
